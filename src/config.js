...
module.exports = {
  ...
  login: {  // 登录相关配置
    getCurrentUser: '/api/staff/getCurrentUser',  // 后端必须要提供接口校验当前用户的身份, 如果拿不到用户信息, 才会尝试登录
    // 登录有两种情况:
    // 1. 使用sso登录, 直接跳转就可以了
  ...
  upload: {  // 上传相关配置
    // 上传图片和上传普通文件分别配置
    image: '/resource/uploadImage',  // 默认的上传图片接口
    imageSizeLimit: 1500,  // 默认的图片大小限制, 单位KB

    file: '/resource/uploadFile',  // 默认的上传文件的接口
  ...
  DBTable: {  // DBTable组件相关配置
    ...
    default: {  // 针对每个表格的默认配置
      ...
      type:'normal',   //normal为普通表格; tree为树形表格;  默认为normal
      lazyMode:false  // type为tree时候生效 (true 时候 每次展开都会加载后台数据 );默认为false
    },
  },
